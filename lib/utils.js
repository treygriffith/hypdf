...
/**
 * Get a Buffer or Readable Stream of file data from a filename
 * @param  {String | Buffer | Readable Stream}   file     the raw Buffer of file data, a stream to the file data, or a filename from which to fetch it
 * @param  {Function} callback evaluated with results
 */
utils.getBuffer = function getBuffer(file, callback) {
  if(file instanceof Buffer || file instanceof stream.Readable || file instanceof stream.Stream) return callback(null, file);

  if(!file || typeof file !== 'string') return callback(new Error("File must be a buffer, a readable stream, or a filename"));
