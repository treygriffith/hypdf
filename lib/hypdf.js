...
HyPDF.prototype.request = function (endpoint, data, callback) {
  ...
  req = request.post(this.host + endpoint, function (err, res, body) {

      // Handle errors
      if(err) return callback(err);
      if(res.statusCode !== 200) return callback(new Error(endpoint + " returned status code "+res.statusCode+((res.body && res.body.message) ? " with message: "+res.body.message : "")));
